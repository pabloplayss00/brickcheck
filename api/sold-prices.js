...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (country === "Northern Ireland") {
      res.status(400).json({
        error:
          "KerbCheck covers England, Wales & Scotland for now — Northern Ireland's sold-price records aren't open data yet. Sorry!",
      });
      return;
    }
    ...
    if (country === "Scotland") {
      ...
      res.status(200).json({ mode: "hpi", postcode, region: hpi.regionName, month: hpi.month, averages: hpi.averages, lat: latitude, lng: longitude });
      return;
    }
...
SELECT ?amount ?date ?propertyType ?paon ?street ?postcode ?category ?status
WHERE {
  ...
  OPTIONAL { ?transx lrppi:transactionCategory ?category }
  OPTIONAL { ?transx lrppi:recordStatus ?status }
...
`);
    ...
    // Drop rows that HM Land Registry itself flags as unreliable for a fair
    // "what did it genuinely sell for" comparison:
    //  - "Additional Price Paid" (Category B) transactions include repossessions,
    //    identified buy-to-lets and transfers to non-private buyers — sales that
    //    are real, but not reliably arm's-length market-value sales.
    //  - Records marked as a "delete" are corrections HM Land Registry has since
    //    superseded (e.g. a price was entered wrong and re-published) and should
    //    no longer count.
    // Matched by substring rather than an exact URI so this stays safe even if
    // the vocabulary's exact casing/format differs from what's expected here —
    // unmatched/unlabelled rows are kept, never dropped by accident.
    const isReliableMarketSale = (r) => {
      const cat = r.category ? r.category.value.toLowerCase() : "";
      const status = r.status ? r.status.value.toLowerCase() : "";
      if (cat.includes("additional")) return false;
      if (status.includes("delete")) return false;
      return true;
    };

    const sales = rows
      .filter(isReliableMarketSale)
      .map((r) => ({
    ...
    res.setHeader("Cache-Control", "s-maxage=86400, stale-while-revalidate=604800");
    res.status(200).json({ mode: "ppd", postcode, count: sales.length, sales, lat: latitude, lng: longitude });
  } catch (err) {
